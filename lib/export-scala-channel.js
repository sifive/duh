...
const masterAdapterGen = comp => e =>
  `IO(new Bundle { // channel master adapter
    val ${e.name} = Decoupled(${type(findPort(comp, e).wire.width)})
  })
`;

const slaveAdapterGen = comp => e =>
  `IO(new Bundle { // channel slave adapter
    val ${e.name} = Flipped(Decoupled(${type(findPort(comp, e).wire.width)}))
  })
`;
...
${
  comp.busInterfaces
    ...
    .concat(
      comp.busInterfaces
        .filter(e => (e.busType.name === 'channel' && e.interfaceMode === 'master'))
        .map(e => `RegFieldGroup("${e.name}",Some("${e.name} initiator channel"), NonBlockingEnqueue(${e.name}_queue.io.enq))`)
    )
    .map((e, i) => `      ${i * 8} -> ${e}`)
    .join(',\n')
}
