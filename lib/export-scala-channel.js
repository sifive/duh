...
const commonNodeGen = comp => {
  ...
  val channelsNode = TLRegisterNode(
    address = Seq(AddressSet(0x10080000L, 0xFFF)),
    device = imp.device,
    beatBytes = 8
  )
...
module.exports = {
  ...
  master: {
    ...
    params: () => e => `case class P${e.name}Params (burstBytes: Int) // channel master params`,
  ...
  slave: {
    ...
    params: () => e => `case class P${e.name}Params (burstBytes: Int) // channel slave params`,
