...
${comp.busInterfaces
    ...
    .map(e => `    imp.channelsNode.${e.name} <> ${e.name}_queue.io.deq`)
    .join('\n')
}

${comp.busInterfaces
    .filter(e => (e.busType.name === 'channel' && e.interfaceMode === 'master'))
    .map(e => `    ${e.name}_queue.io.enq <> imp.channelsNode.${e.name}`)
    .join('\n')
}
...
${
  comp.busInterfaces
    .filter(e => (e.busType.name === 'channel' && e.interfaceMode === 'slave'))
    .map(e => `RegFieldGroup("${e.name}",Some("${e.name} target channel"), NonBlockingEnqueue(${e.name}_queue.io.deq)))`)
    .concat(
      comp.busInterfaces
        .filter(e => (e.busType.name === 'channel' && e.interfaceMode === 'master'))
        .map(e => `Seq(RegGroup("${e.name}",Some("${e.name} initiator channel"), NonBlockingDequeue(${e.name}_queue.io.enq)))`)
    )
    .map((e, i) => `      ${i * 8} -> ${e}`)
    .join('\n')
}
