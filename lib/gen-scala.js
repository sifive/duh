...
${indent(4)(comp.busInterfaces.map(e =>
  ...
  def setBurstBytes(x: Int) = this.copy(
${indent(4)(comp.busInterfaces.map(e =>
        `${e.name}Params = ${e.name}Params.copy(setBurstBytes = x)`
    ).join(',\n'))}
  )
}

object N${comp.name}TopParams {
  def defaults(control: BigInt = 0x60000, burstBytes = 64) =
    N${comp.name}TopParams(
      blackbox = BBParams(control),
${indent(6)(comp.busInterfaces.map(e =>
        `${e.name}Params = ControlParams(burstBytes)`
    ).join(',\n'))}
    )
}

// stub: class N${comp.name}Top extends N${comp.name}TopBase
class NameTop(c: N${comp.name}TopParams)(implicit p: Parameters) extends LazyModule
{
...
class WithNameTop extends Config((site, here, up) => {
