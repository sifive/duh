...
const slaveAdapterGen = () => {
    const address = 'List(AddressSet(c.control, 0x400L))';
...
val ${e.name}Node: TLInwardNode =
  (imp.${e.name}Node
    := AXI4Buffer(
      aw = c.controlParams.axi4BufferParams.aw,
      ar = c.controlParams.axi4BufferParams.ar,
      w = c.controlParams.axi4BufferParams.w,
      r = c.controlParams.axi4BufferParams.r,
      b = c.controlParams.axi4BufferParams.b)
    := AXI4UserYanker(capMaxFlight = Some(c.controlParams.maxTransactions))
    := TLToAXI4()
    := TLFragmenter(/*axi4-lite data bus width in bytes*/, c.controlParams.burstBytes)
    := TLBuffer(
      a = c.controlParams.tlBufferParams.a,
      b = c.controlParams.tlBufferParams.b,
      c = c.controlParams.tlBufferParams.c,
      d = c.controlParams.tlBufferParams.d,
      e = c.controlParams.tlBufferParams.e))
`;

const slaveNodeGen = masterNodeGen;

const masterAttachGen = e =>
    `name.${e.name}Node := TLWidthWidget(pbus) := pbus`;
