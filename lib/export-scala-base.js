...
const perBusInterface = e => {
  ...
  const handle =
    generator[e.interfaceMode].adapter || (() => 'unknownBusType()');
...
${indent(2)(
    comp.busInterfaces
      .map(e => generators[e.busType.name][e.interfaceMode].node(e))
      .join('\n')
  )}
}
...
${indent(4)(
    comp.busInterfaces
      .map(e => generators[e.busType.name][e.interfaceMode].attach(e))
      .join('\n')
  )}
