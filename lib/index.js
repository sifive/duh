...
const genPA = require('./export-pa.js');
...
exports.pa = async argv => {
  if (argv.verbose) console.log('generate');
  const duh = await readDuh(argv);
  const dir = argv.output;

  const setupFile = `${dir}/${duh.component.name}_setup.sh`;
  await fs.outputFile(setupFile, genPA.setup(duh));

  const buildFile = `${dir}/${duh.component.name}_build.sh`;
  await fs.outputFile(buildFile, genPA.build(duh));

  const tclFile = `${dir}/${duh.component.name}_create.tcl`;
  await fs.outputFile(tclFile, genPA.tcl(duh));
};
...
exports.scala = async argv => {
  ...
  const dir = argv.output; // moduleName; // argv.dir;
  ...
  // generate .h file
  const headerFile = `${dir}/src/${duh.component.name}.h`;
