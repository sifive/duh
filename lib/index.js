...
exports.generate = async argv => {
    ...
    await outputFile(`build/${duh.component.name}.json5`, JSON5.stringify(duh, null, 2));
    // generate expanded IP-XACT
    // TODO
    // generate Scala wrapper
    await outputFile(`build/${duh.component.name}.scala`, genScala(duh));
    // generate verilog black box
    await outputFile(`build/${duh.component.name}-bbx.v`, genVerilogBBX(duh));
    // generate Spirit
    await outputFile(`build/${duh.component.name}.xml`, genSpirit(duh));
